...
### 必要要件
- NVIDIA GPU（CUDA）または Apple Silicon（MPS）
- Python 3.11+、Node.js 18+

### インストール

```bash
git clone https://github.com/toorpia-labs/llm-hidden-state-platform.git
cd llm-hidden-state-platform
```
...
sudo systemctl status llm-platform-backend llm-platform-frontend

# macOS
deploy/macos/service.sh stop
deploy/macos/service.sh status
```

### モデルの追加

`backend/models.yaml` にエントリを追記してバックエンドを再起動するだけです。コード変更不要。

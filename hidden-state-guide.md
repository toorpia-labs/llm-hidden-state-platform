...
> 利用可能なモデルはサーバーのVRAM容量に依存します。大規模モデルを使いたい場合はVRAMの多いGPUを用意してください。
...
<br>
現在のUIでは1回の実行につき1つのlayerのみ取得できます。異なるlayerを比較したい場合は、同じプロンプトで抽出を複数回実行し、それぞれのCSVをtoorPIAで別々のbasemapとして作成してください。
<br><br>
</details>
